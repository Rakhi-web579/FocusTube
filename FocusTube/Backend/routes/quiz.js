...
// // ---------------------------------------------------------------------------
// // AI-POWERED QUIZ GENERATION via DeepSeek API
// // Get your API key at: platform.deepseek.com
// // Add to .env: DEEPSEEK_API_KEY=sk-...your_key_here
// // ---------------------------------------------------------------------------

// async function generateQuizWithAI(topic, videoTitle) {
//   const apiKey = process.env.DEEPSEEK_API_KEY;
//   if (!apiKey) throw new Error('DEEPSEEK_API_KEY not set');

//   const prompt = `You are a quiz generator for an educational video platform.

// Generate exactly 5 multiple-choice quiz questions for a student who just watched a YouTube video about: "${videoTitle || topic}"
// The study topic/goal is: "${topic}"
...
// Respond with ONLY a raw JSON array — no markdown, no backticks, no preamble.
// Format:
// [
//   {
//     "question": "...",
...
//     throw new Error('DeepSeek returned invalid quiz format');
//   }
...
// // ---------------------------------------------------------------------------
// // FALLBACK QUIZ BANK — used when DeepSeek is unavailable
// // ---------------------------------------------------------------------------
...
//   try {
//     console.log(`🤖 Generating DeepSeek quiz for: "${video_title || topic}"`);
//     rawQuestions = await generateQuizWithAI(topic, video_title);
//     console.log(`✅ DeepSeek quiz generated successfully`);
//   } catch (err) {
//     console.warn(`⚠️  DeepSeek quiz generation failed:`, err.response?.data || err.message);
//     source = 'fallback';
//     rawQuestions = findQuizForTopic(topic, video_title || '');
//   }
...
//   res.json({ quiz, topic, total: quiz.length, source });
// });
...
//   if (!answers || !quiz) {
//     retonst userAnswer = answers[index];
//     curn res.status(400).json({ error: 'answers and quiz are required' });
//   }
...
// And in your server entry file:
//   require('dotenv').config();
// Install SDK with: npm install @google/generative-ai
// ─────────────────────────────────────────────
const { GoogleGenerativeAI } = require('@google/generative-ai');
const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY);

// ─────────────────────────────────────────────
// Fallback quiz bank (used when Gemini fails)
// ─────────────────────────────────────────────
const QUIZ_BANK = {
  'binary search': [
    {
...
// ─────────────────────────────────────────────
// Fallback: Find the best matching quiz for a topic
// ─────────────────────────────────────────────
function findQuizForTopic(topic, videoTitle) {
  const searchText = `${topic} ${videoTitle}`.toLowerCase();
...
// ─────────────────────────────────────────────
// Helper: shuffle options and recalculate correct index
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// Helper: call Gemini and parse the response safely
// ─────────────────────────────────────────────
async function generateWithGemini(topic, videoTitle) {
  const model = genAI.getGenerativeModel({ model: 'gemini-2.5-flash' });

  const prompt = `
You are a quiz generator. Generate exactly 5 multiple choice questions about "${topic}" based on a video titled "${videoTitle || topic}".

IMPORTANT: Return ONLY a raw JSON array. No markdown, no code blocks, no explanation, no extra text before or after.

Each object in the array must have exactly these fields:
- "question": string
- "options": array of exactly 4 strings
- "correct": number (0-3), the index of the correct option in the options array
...
[{"question":"...","options":["A","B","C","D"],"correct":0,"explanation":"..."}]
  ...
  // Strategy 3: extract the first [...] array found in the text
  const match = rawText.match(/\[[\s\S]*\]/);
  if (match) {
    return JSON.parse(match[0]);
  }

  // If all strategies fail, throw so we fall back to the quiz bank
  throw new Error(`Gemini returned unparseable response: ${rawText.slice(0, 200)}`);
}
...
router.get('/generate', async (req, res) => {
  ...
  if (process.env.GEMINI_API_KEY) {
    try {
      const questions = await generateWithGemini(topic, video_title || '');

      // Basic validation: make sure we got an array of questions
      if (!Array.isArray(questions) || questions.length === 0) {
        throw new Error('Gemini returned empty or invalid array');
      }

      const quiz = formatQuestions(questions.slice(0, 5));
      return res.json({ quiz, topic, total: quiz.length, source: 'gemini' });

    } catch (err) {
      // Log the error but don't crash — fall through to hardcoded bank
      console.error('Gemini quiz generation failed, using fallback:', err.message);
    }
  }

  // Fallback: use hardcoded quiz bank
  const allQuestions = findQuizForTopic(topic, video_title || '');
  const shuffled = [...allQuestions].sort(() => Math.random() - 0.5);
  const selected = shuffled.slice(0, Math.min(5, shuffled.length));
  const quiz = formatQuestions(selected);

  return res.json({ quiz, topic, total: quiz.length, source: 'fallback' });
});
